...
module SolidusStripeV5::CheckoutTestHelper
  ...
  def payment_method
    ...
    @payment_method ||= SolidusStripeV5::PaymentMethod.first!
  end
  ...
  def last_stripe_payment
    current_order.payments.reorder(id: :desc).find_by(source_type: "SolidusStripeV5::PaymentSource")
  end

  def capture_last_valid_payment
    payment = current_order.payments.valid.last
    payment.capture!
    expect(payment.payment_method.type).to eq('SolidusStripeV5::PaymentMethod')
  ...
  def payment_intent_is_created_with_required_capture
    intent = SolidusStripeV5::PaymentIntent.where(
